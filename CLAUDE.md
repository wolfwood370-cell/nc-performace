...
10. **Codice snello**: niente file >300r monolitici nuovi · niente import non usati · niente dead code · niente `console.log` nuovi (usa `src/lib/logger.ts`) — **è una convenzione, non un cancello**: eslint la segnala e la CI non la boccia, per la scelta scritta a `eslint.config.js:52-53`.
