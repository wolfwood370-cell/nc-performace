...
| ---------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tabelle `public` | **53** (52 dal 2026-09-05: `coach_knowledge_base` rimossa) | 52 in `types.ts` + `appointments` (assente da `types.ts` per regen Lovable, presente come migration).                                                                                                                     |
...
| Extension        | **`vector`** (pgvector)                                    | Indispensabile per RAG (una libreria, `match_knowledge_chunks` su `knowledge_chunks`; `match_documents` e `coach_knowledge_base` rimosse il 2026-09-05). Va abilitata sul nuovo progetto **prima** del restore.           |
...
| Elemento                    | Conteggio                                                                                     | Note                                                                                                                                                                                                                                                                                                                                                |
| --------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Funzioni Postgres           | **25** (24 dal 2026-09-05: `match_documents` rimossa)                                         | Incl. helper `SECURITY DEFINER` per RLS anti-ricorsione (`is_room_member`, `shares_room_with`, `is_coach_of_athlete`, `is_my_athlete`, `is_my_coach`), RAG (una libreria, `match_knowledge_chunks`; `match_documents` rimossa il 2026-09-05), clone/schedule programmi, `handle_new_user`, `prevent_profile_privilege_escalation`, watchdog/notify. |
