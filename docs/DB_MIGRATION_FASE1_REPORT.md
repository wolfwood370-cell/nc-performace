...
| **ADV** | Advisor security (WARN)                             | 0 ERROR. WARN su _SECURITY DEFINER executable_ (anon/auth), _public bucket listing_ (coach-logos/avatars/branding), _search_path mutable_ (`set_appointments_updated_at`): **stessi findings deferiti del sorgente**. Ownership = Decisione **D5** (da ridefinire senza Lovable Security Agent).                     | report-only                |
...
Leggi prima: CLAUDE.md + .claude/methodology/00-CORE.md + 03-BACKEND-LOVABLE.md, e
