...
> **Aggiornato:** 2026-07-15 (Claude Code, fetta allenamento-autonomo) — **stato vivo nell'Addendum §0 qui sotto**, che prevale su §2/§4 dove diverge; fotografia completa in `docs/stato-repo-2026-07-12.md`. Stato 2026-06-18: Migrazione Lovable→Supabase di proprietà avanzata: smoke AI 6/6, flusso invito→atleta verificato, fix cache roster, **`generate-program` cablata nella UI (D11)**, **D12 Set A pulizia dead-code ACWR fatto**, **dual-agent Code/Cowork formalizzato (`COWORK.md`, `69e8a4a`)**, **campagna autonoma D13 COMPLETA** (7 hook WIP collegati + E2E verde + audit pulito). Codice su `origin/main` (tip `2296074`).
...
## 0. Addendum 2026-07-15 (stato vivo — prevale su §2/§4 e sugli addendum precedenti dove diverge)
...
- **⚠ BLOCCO GO-LIVE autonomo — consenso `ai_processing`**: esiste nell'enum DB ma è "parked" nella raccolta intake (`validateSpec.ts CONSENT_TYPES` = 6). Serve la fetta "un-park + raccolta `ai_processing` all'intake" (+ track legale di Nick sul testo). Fino ad allora ogni atleta reale riceve `{consentRequired:true, missing:['ai_processing']}` — comportamento art. 22 corretto. Per l'atleta di test: seed della riga `ai_processing` via connettore (Cowork).
