...
| 10  | **Droppare i 3 stash GHD residui** (`coach-guard`, `e2e-native`, `oauth-migration`) **solo dopo** che Nick ha messo al sicuro il `.env` in `stash oauth-migration`. I doc unici sono già stati recuperati e trackati (`1de3b7f`); negli stash restano `.env` (secret) + artefatti Playwright (rigenerabili).                                                                                                           | 👤 + CC           |
