...
4. **Variante (c) sul major di review (decisione Nick 2026-07-19, post-implementazione)**: il magiclink-as-lookup prescritto sopra (§ARCHITETTURA) ruotava il `recovery_token` GoTrue del bersaglio anche sui rami senza email (409 incluso). Nick ha **revocato il divieto assoluto di `listUsers`** (motivato dal limite perPage:200, caduto con la paginazione completa): lookup read-only `listUsers` paginato completo; `generateLink(magiclink)` SOLO nei rami che inviano, DOPO `decideAlreadyExists` e le scritture profilo (invio fallito → 502, retry idempotente su resend-pending); rami no-send e 409 a zero side-effect auth. L'acceptance «`listUsers` = 0» dell'addendum §2 è SUPERATA da: `listUsers` solo nel lookup paginato con doc-comment read-only · nessun magiclink prima della decisione · raggiungibilità no-send verificata. Il §ARCHITETTURA originale resta sopra come storia della spec.
