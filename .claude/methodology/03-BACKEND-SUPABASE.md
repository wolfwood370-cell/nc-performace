...
> ⛔ **Questo §0 NON sono gli invarianti clinici.** Qui si parla di chi possiede RLS, apply delle migration e advisor. Gli **invarianti clinici** sono undici, `§0.1`…`§0.11`, e stanno in `app/spec-CORE-2026-07-11.md` §0 (fuori dal repo).
...
| **Claude Code** | **Codice sicuro** + `/security-review` ai milestone. Propone migration/policy come **FILE** in `supabase/migrations/`, **non applica sul DB**: l'MCP Supabase montato in Code è `--read-only` (`.mcp.json:3-14`) — legge schema/advisor/SELECT, non scrive (`CLAUDE.md` legge #11). |
...
| Oggetto                                                          | Cosa                                                                                                                                                                                                                                                                                                                               | RLS                                                                                                                                                                                  |
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `profiles.coaching_mode` enum {coached, autonomous}              | modalità del cliente; il gate §0 del CORE la legge per scegliere il percorso di rilascio — `CORE §0.1` (stesso motore, cambia CHI rilascia: Coached = Nicolò valida ogni bozza · Autonoma = il motore applica entro i guardrail ed escala su qualsiasi segnale clinico). ≠ `mode` body-param di `generate-program` (new\|continue) | eredita `profiles`                                                                                                                                                                   |
...
Oltre a `workout_logs` + `exercise_logs`, l'anello atleta scrive **`daily_readiness`**: check-in giornaliero (UNIQUE `athlete_id,date`; upsert da `src/hooks/athlete/useAthleteReadinessHooks.ts` — usato da `DailyCheckin.tsx` — e da `useOfflineSync.ts`) con readiness `score`, sonno/stress/energia/umore/fatica/digestione, `body_weight`, `has_pain` + `soreness_map` (jsonb per muscolo). Letto da: dashboard coach (alert `low_readiness`, soglie 45/50 in `useCoachDashboardMetrics`/`useCoachData`), risk overview, analytics (`body_weight`), health-profile (`has_pain`/`soreness_map`). **`has_pain`/`soreness_map` = l'input della cattura-sicurezza per-ciclo dell'anello Autonomo — `CORE §0.7`, che non è una descrizione ma una precondizione: «nessun input di sicurezza ⇒ nessun rilascio».** Oggi §0.7 è cablato **solo** nell'anello nutrizione (`supabase/functions/compute-nutrition-target/target/processAthlete.ts:163-181`, `has_pain === true` → esito `safety_capture`); l'anello allenamento **non legge `daily_readiness`** — `release-autonomous-program` è idempotente sul primo rilascio (`index.ts:8`, `supersedes_id: null` a `:372`), quindi §0.7 non è ancora esigibile lì. **Chi cabla il ciclo N+1 dell'allenamento autonomo deve cablare §0.7 nella stessa fetta**, o l'auto-release nasce senza cattura-sicurezza. Debiti nominati (fetta allenamento-autonomo): niente `updated_at` (la conflict-resolution offline usa `created_at`) e `score` placeholder-85 scritto da `DailyCheckin.tsx`.
